...
def snr_mixer(clean: np.ndarray, noise: np.ndarray, snr: float) -> np.ndarray:
    """
    returns a mix of form (clean + alpha*noise) such that
    SNR(clean, mix) = snr
    """
...
def create_mix(idx: int, triplet: Dict, snr_levels, out_storage: MixturesStorage,
    ...
    """
    :param snr_levels: from these values the target mix snr is chosen randomly
    :param idx: numerical index of the mix
    :param vad_db: a threshold of silence for mixing two audios
    :param audio_len: audios length in seconds for mix
    ...
    if not test:
        ...
        for i in range(len(s1_cut)):
            ...
            sf.write(cur_out_filepaths['mix'], mix, sr)
            sf.write(cur_out_filepaths['target'], s1_cut[i], sr)
            sf.write(cur_out_filepaths['ref'], ref, sr)
        ...
        sf.write(out_filepaths['mix'], mix, sr)
        sf.write(out_filepaths['target'], s1, sr)
        sf.write(out_filepaths['ref'], ref, sr)
...
class MixtureGenerator:
    ...
    def generate_triplets(self) -> Dict[str, List]:
        """
        generate triplets of files (ref, target, noise)
    ...
    def triplet_generator(target_speaker: SpeakerFiles,
        ...
        """
        generate `number_of_triplets` triplets for particular target and noise speakers
        """
